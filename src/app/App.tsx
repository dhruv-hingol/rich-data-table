import { lazy, Suspense } from "react";
import { Toaster } from "sonner";
import { AppRouter } from "@/src/routes/appRouter";

const FilterSheet = lazy(
  () => import("@/src/features/inventory/components/FilterSheet"),
);
const BulkImportDialog = lazy(
  () =>
    import("@/src/features/inventory/components/bulk-import/BulkImportDialog"),
);

export default function App() {
  return (
    <div className="h-screen w-screen max-h-screen max-w-screen overflow-hidden bg-[#fff] text-slate-900 flex flex-col font-sans antialiased">
      <Toaster position="top-center" richColors closeButton />
      <main className="flex-1 max-w-[1920px] w-full mx-auto flex flex-col min-h-0 overflow-y-auto px-2 sm:px-4 md:px-6 pb-4 sm:pb-4 md:pb-4 pt-0">
        <AppRouter />
      </main>
      <Suspense fallback={null}>
        <FilterSheet />
        <BulkImportDialog />
      </Suspense>
    </div>
  );
}
